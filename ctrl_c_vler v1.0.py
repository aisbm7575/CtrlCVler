...
# [추가됨] QIcon 임포트
from PyQt6.QtGui import QDrag, QFont, QDesktopServices, QIcon
from PyQt6.QtNetwork import QLocalServer, QLocalSocket 

DATA_FILE = "data.json"
REG_APP_NAME = "CtrlCVler"
IPC_SERVER_NAME = "CtrlCVler_IPC_Server"

# [추가됨] PyInstaller 환경에서도 리소스(아이콘) 경로를 찾게 해주는 마법의 함수
def resource_path(relative_path):
    """ Get absolute path to resource, works for dev and for PyInstaller """
    try:
        # PyInstaller가 임시 폴더에 압축을 풀었을 때의 경로
...
class MouseHookThread(QThread):
    # ... (기존과 동일하므로 생략 없이 원래 코드 유지) ...
    def __init__(self):
        super().__init__()
        self.right_press_time = 0
        self.middle_press_time = 0
        self.is_right_holding = False
        self.is_middle_holding = False
    ...
    def on_click(self, x, y, button, pressed):
        if button == mouse.Button.right:
            if pressed: 
                ...
                self.is_right_holding = True
            else:
                if not self.is_right_holding:
                    return
                self.is_right_holding = False
                if time.time() - self.right_press_time >= 0.5:
                    self.trigger_paste(use_esc=True)
        elif button == mouse.Button.middle:
            if pressed: 
                self.middle_press_time = time.time()
                self.is_middle_holding = True
            else:
                if not self.is_middle_holding:
                    return
                self.is_middle_holding = False
                if time.time() - self.middle_press_time < 1.0: 
                    self.trigger_paste(use_esc=False)

    def trigger_paste(self, use_esc):
        if self.is_pasting:
            return
        self.is_pasting = True
        try:
            print(f"[{time.strftime('%H:%M:%S')}] 붙여넣기 실행됨!") 
            if use_esc:
                keyboard.send('esc') 
                time.sleep(0.1)
            keyboard.press('ctrl')
...
class CtrlCVlerApp(QMainWindow):
    ...
    def setup_ipc_server(self):
        ...
        self.local_server.removeServer(IPC_SERVER_NAME)
        self.local_server.listen(IPC_SERVER_NAME)
        self.local_server.newConnection.connect(self.wake_up_window)
    ...
    def initUI(self):
        ...
        # [추가됨] 프로그램 타이틀 바(좌측 상단) 및 작업 표시줄 아이콘 설정
        ...
        # ... (나머지 initUI 코드는 기존과 동일하므로 생략 없이 유지) ...
    ...
    def setup_tray(self):
        ...
        # [수정됨] 기본 아이콘 대신 준비한 icon.ico 파일 적용
    ...
    # ... (나머지 클래스 메서드는 기존과 완벽히 동일하므로 그대로 사용) ...
...
if __name__ == '__main__':
    ...
    if socket.waitForConnected(500):
        print("이미 프로그램이 실행 중입니다. 기존 창을 활성화합니다.")
        sys.exit(0)
